...
"""
    ...
    def arg_tokenize(self, argstr):
        return [
            unicode(token, "utf-8") for token in shlex.split(argstr.encode("utf-8"))
        ] or [""]
    ...
    def help_add(self):
        print(
            u"add <torrent file or url> [<target dir> paused=(yes|no) peer-limit=#]\n"
        )
    ...
    def help_info(self):
        ...
        print(
            u"Get details for a torrent. If no torrent id is provided, all torrents are displayed.\n"
        )
...
def main(args=None):
    ...
    commands = [
        cmd[3:] for cmd in itertools.ifilter(lambda c: c[:3] == "do_", dir(Helical))
    ]
    address = "localhost"
    port = DEFAULT_PORT
    command = None
    ...
    if command:
        command_args = u" ".join(
            [u'"%s"' % arg for arg in args[args.index(command) + 1 :]]
        )
