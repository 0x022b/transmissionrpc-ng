            ...
            if len(errors) > 0:
                ...
                raise Exception(
                    ...
                    % (
                        ...
                        json.dumps(test_data["request"], indent=2),
                        errors,
                    )
                )
...
class ClientTest(unittest.TestCase):
    def testConstruction(self):
        ...
        self.assertEqual(
            tc.url,
            "http://localhost:%d/transmission/rpc"
            % (transmissionrpc.constants.DEFAULT_PORT),
        )
        tc = createClient(
            "127.0.0.1", 7000, user="user", password="secret", test_name="construction"
        )
        ...
        self.assertEqual(tc.url, "http://127.0.0.1:7000/transmission/rpc")
        tc = createClient(
            "127.0.0.1", 7000, password="secret", test_name="construction"
        )
        self.assertEqual(tc.url, "http://127.0.0.1:7000/transmission/rpc")
        tc = createClient(
            "127.0.0.1", 7000, password="secret", timeout=0.1, test_name="construction"
        )
        self.assertEqual(tc.url, "http://127.0.0.1:7000/transmission/rpc")
        self.assertEqual(tc.timeout, 0.1)
        tc = createClient(
            "127.0.0.1", 7000, password="secret", timeout=10, test_name="construction"
        )
    ...
    def testAddTorrent(self):
        ...
        torrent_path = os.path.join(
            data_path, "ubuntu-12.04.2-alternate-amd64.iso.torrent"
        )
        ...
        r = tc.add_torrent(
            "torrent.txt", paused=False, download_dir="/var/downloads", peer_limit=1
        )
        self.assertEqual(r.id, 0)
        ...
        r = tc.add_torrent(
            "file://" + os.path.join(data_path, "torrent.txt"),
            paused=True,
            download_dir="/tmp",
            peer_limit=200,
        )
        ...
        r = tc.add_torrent(
            "file://"
            + os.path.join(data_path, "ubuntu-12.04.2-alternate-amd64.iso.torrent")
        )
        ...
        r = tc.add_torrent(
            "file://"
            + os.path.join(data_path, "ubuntu-12.04.2-alternate-amd64.iso.torrent.gz")
        )
    ...
    def testGetTorrentsHashes(self):
        ...
        r = tc.get_torrents(
            [
                ...
                "a33e98826003515e46ef5075fcbf4914b307abe2",
            ]
        )
        for torrent in r:
            if torrent.id == 2:
        ...
        tc = createClient(test_name="get_torrents_hashes")
        r = tc.get_torrents(
            "ab8ea951c022d4745a9b06ab8020b952a52b71ca,a33e98826003515e46ef5075fcbf4914b307abe2"
        )
        for torrent in r:
            if torrent.id == 2:
                self.assertEqual(torrent.name, "ubuntu-10.04-server-amd64.iso")
    ...
    def testParseId(self):
        ...
        self.assertEqual(
            ...
            "a21c45469c565f3fb9595e4e9707e6e9d45abca6",
        )
    ...
    def testParseIds(self):
        ...
        self.assertEqual(
            parse_torrent_ids("a21c45469c565f3fb9595e4e9707e6e9d45abca6"),
            ["a21c45469c565f3fb9595e4e9707e6e9d45abca6"],
        )
